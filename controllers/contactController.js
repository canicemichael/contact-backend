...
//@access private
// the user that we found with .find() will be getting all the contacts they created
...
const updateContact = asyncHandler(async (req, res) => {
  ...
  // this is to check for the user who created the contact is
  // the current user who is updating it
...
const deleteContact = asyncHandler(async (req, res) => {
  ...
  // this is to check for the user who created the contact is
  // the current user who is deleting it
  if (contact.user_id.toString() !== req.user.id) {
    res.status(403);
